...
export async function POST(request: NextRequest) {
  try {
    ...
    if (authError || !user) {
      console.error("[v0] Auth failed in approve endpoint:", authError?.message)
    ...
    console.log("[v0] Processing approval:", { request_id, approved, manager_id: user.id })
    ...
    if (approved) {
      ...
      const staffName = pendingRequest.user_profiles 
        ? `${pendingRequest.user_profiles.first_name} ${pendingRequest.user_profiles.last_name}`
        : "Staff Member"
